...
async def list(request: Request, token: str = Depends(Protect), db: AsyncSession = Depends(get_async_db), commons: CommonDependencies = Depends(CommonDependencies)):
    ...
    try:
        r = await StadiumModel.objects(db).all(offset=commons.offset, limit=commons.size)
        return {
            'data': r,
...
async def get(request: Request, stadium_id: str, db: AsyncSession = Depends(get_async_db), token: str = Depends(Protect)):
    token.auth(['admin', 'manager', 'user'])
    try:
        result = await StadiumModel.objects(db).get(id=stadium_id)
        if result:
            return result
        else:
...
async def create(request: Request, stadium: CreateStadium, db: AsyncSession = Depends(get_async_db), token: str = Depends(Protect)):
    token.auth(['admin']) 
    try:
        await StadiumModel.validate_unique_name_location(db, stadium.name, stadium.location)
        ...
        new_stadium = await StadiumModel.objects(db).create(**kwargs)
        return new_stadium
    except HTTPException as e:
        raise e
...
async def create_multiple_stadiums(request: Request, stadiums: List[CreateStadium], db: AsyncSession = Depends(get_async_db), token: str = Depends(Protect)):
    token.auth(['admin']) 
    ...
    try:
        for stadium_index, stadium in enumerate(stadiums):
            try:
                ...
                new_stadiums = await StadiumModel.objects(db).create(only_add=True, **kwargs)
...
async def upsert_multiple_stadiums(request: Request, stadiums: List[UpsertStadium], db: AsyncSession = Depends(get_async_db), token: str = Depends(Protect)):
    token.auth(['admin'])
    new_items, errors_info = [], []
    try:
        for stadium_index, stadium in enumerate(stadiums):
            try:
                ...
                if new_data['id']:
                    old_data = await StadiumModel.objects(db).get(id=new_data['id'])
                    kwargs['signal_data']['old_data'] = old_data.__dict__ if old_data else {}

                    await StadiumModel.objects(db).update(obj_id=new_data['id'], **kwargs)
                    updated_deployments = await StadiumModel.objects(db).get(id=new_data['id'])
...
async def update(request: Request, stadium_id: Union[str, int], stadium: CreateStadium, db: AsyncSession = Depends(get_async_db), token: str = Depends(Protect)):
    token.auth(['admin'])
    try:
        await StadiumModel.validate_unique_name_location(db, stadium.name, stadium.location, stadium_id)
        old_data = await StadiumModel.objects(db).get(id=stadium_id)
        ...
        await StadiumModel.objects(db).update(obj_id=stadium_id, **kwargs)
        result = await StadiumModel.objects(db).get(id=stadium_id)
        return result
    except HTTPException as e:
        raise e
...
async def delete(request: Request, stadium_id: Union[str, int], db: AsyncSession = Depends(get_async_db), token: str = Depends(Protect)):
    token.auth(['admin'])
    try:
        kwargs = {
            "model_data": {},
        ...
        await StadiumModel.objects(db).delete(obj_id=stadium_id, **kwargs)
    except FileNotFoundError:
        raise HTTPException(status_code=404, detail={
                "field_name": "{stadium_id}",
                "message": f"<{stadium_id}> record not found in  stadiums"
...
async def delete_multiple_stadiums(request: Request, stadiums_id: List[str] = QueryParam(), db: AsyncSession = Depends(get_async_db), token: str = Depends(Protect)):
    token.auth(['admin'])
    kwargs = {
        "model_data": {},
        "signal_data": {
    ...
    await StadiumModel.objects(db).delete_multiple(obj_ids=stadiums_id, **kwargs)
